...
double COptimisation::objective( boolean& isCapacityExceeded,
                                 int&     l, double   t,
                                 int      numberOfSectors,
...
void COptimisation::bestServers( List<CPoint3d>&     centres,
                                 CBaseTransceiver&   btsObject,
                                 const CBuilding&    buildingObject,
                                 const CCapacity&    capacityGrid )
{
  ...
  for ( j = 0; j < numberOfServers; j++ )
  {
    ...
    rayTracingObject = CRayTracing( buildingObject, btsObject, 
                                    pathLossGrid, receiverPointArray, j,
                                    LOSSLESS ); 
...
double COptimisation::objectiveFunction( 
			           ...
			           complex           ***fieldStrength )
{
  ...
  for ( n = 0; n < capacityGrid.yNumberOfPoints(); n++ )
  {
        ...
        /*if ( centresArray[i] == oldCentresArray[i] )
	{
      ...
      for ( i = 0; i < numberOfServers; i++ )
      {
        totalField[p][i] = 0.0;
        for ( k = 0; k < numberOfSectors; k++ )
          totalField[p][i] += qsqr( cabs( gain[i][k] ) 
	                    * cabs(fieldStrength[p][i][k])/ TwoK); 
      ...
      coutput << newSIR[p] << " > " << targetSIR[p] << "?" << endl; 
  ...
  if ( writeBestServer == true )
    matlabObject.pathLoss( pathLossGrid, "bestserver2.txt" );
...
List<CPoint3d>& COptimisation::vectorToCentres(const CVector<double>& vcentres)
{
  List<CPoint3d>  *centres;
  int             i, k, N;
  double          x, y, z;
...
CVector<double> COptimisation::convertCentresToVector( 
                                   List<CPoint3d>& centres )
{
...
CVector<double> COptimisation::gradient( 
                                       double            smallerSize,
                                       double            t,
                                       int               numberOfSectors,
...
boolean COptimisation::annealing(      double            smallerSize,
                                       double            t,
                                       int               numberOfSectors,
                                       CVector<double>&  oldCentres,
                                       CBaseTransceiver& btsObject,
...
CVector<double> COptimisation::steepestDescentSUMT(
                                      List<CPoint3d>&     centres,
  ...
  for ( i = 0; i < numberOfIterations; i++ )
  {
    ...
    while ( gradfx.abs() > eta )
    {
              ...
              || isCapacityExceeded == true )
      {
        ...
        //if ( buildingObject.locateConvexNumber( pt, i ) == false )
          //continue;
...
boolean COptimisation::testFeasibility( CVector<double>& x, int cellIndex,
...
void COptimisation::useObjectiveToCreateConvexConstraintMatrix( 
...
void COptimisation::formConvexOptimisationMatrix( 
                            CMatrix<double>& A, CVector<double>& b,
                            boolean& isCapacityExceeded,
                            int&   l,                double t,
                            double *maximumPower,    complex **gain,
    ...
    for ( m = 0; m < capacityGrid.xNumberOfPoints(); m++ )
    {
      ...
      if ( newSIR > targetSIR[p] )
      {
        ...
        for ( i = 0; i < numberOfServers; i++ )
        {
          ...
          {
            for ( k = 0; k < numberOfSectors; k++ )
              A.set( -qsqr( cabs( fieldStrength[i][k] / TwoK ) ), 
                     l, i*numberOfSectors+k );
          }
          else
          {
            for ( k = 0; k < numberOfSectors; k++ )
              A.set( targetSIR[p] * qsqr( cabs( fieldStrength[i][k] / TwoK ) ), 
                     l, i*numberOfSectors+k );
          }
        }
                                      ...
                                      complex         *Rsectors )
{
  int            N, i, k, size2;
  double         **E;
  double         stepsize1, stepsize2, temp;
  ...
  for ( i = 0; i < N; i++ )
  {
    ...
    for ( k = 0; k < numberOfSectors; k++ )
    {
      if ( AOD <= ( k + 1 ) * sectorAngle )
      {
        Ec[k][i] = fieldStrength;
	temp     = Ec[k][i].real();
	sumA[k] += temp;
	sumB[k] += Ec[k][i].imag();
	sqrSum[k]  += qsqr( temp ); // variance with zero mean
        k        = numberOfSectors;
      }
    }
    for ( k = 0; k < numberOfSectors; k++ )
  ...
  for ( k = 0; k < numberOfSectors; k++ )
  {
    ...
    for ( i = 1; i < size2; i++ )
    {
      Wn = trapezoidal( E[k], N, r, 0, stepsize1 );
...
void   COptimisation::sort( complex* totalField,     int* reorderedIndices,
                            int     numberOfServers )
{
  int            i, j, k, l;
  CMemoryStorage storageObject;
  ...
  /*j = reorderedIndices[0];
  for ( i = 1; i < numberOfServers; i++ )
  {
    k = reorderedIndices[i];
    if ( dTotalField[j] < dTotalField[k] )
    {
      cerr << "Error in sorted fields" << endl;
      cerr << "order( " << i-1 << "):" << dTotalField[j] << endl;
      cerr << "order( " << i << "):" << dTotalField[k] << endl;
      exit(1);
    }
    j = k;
  }*/
...
boolean   COptimisation::offloadServer( int     **bestServerMatrix, 
                                        const   CCapacity& capacityGrid,
					        double     sensitivity,
                                                double     *totalCapacity,
						int        *currentBestServer,
...
double COptimisation::SIR( complex *totalField, int    numberOfServers,
...
void COptimisation::setTargetSIR( const   CCapacity&        capacityGrid,
                                  const   SystemParameters& parameterObject,
                                  double* targetSIR ) 
{
  int              m, n, p;
