...
Ei     = components2( 'incidentfield.txt',    M, N, 3 );
Er     = components2( 'reflectionfield.txt',  M, N, 3 );
Ed     = components2( 'diffractionfield.txt', M, N, 3 );
...
for m = 1:M
  for n = 1:N
    ...
    normedEt( m, n )     = sqrt( Et(m, n, 5 )^2 + Et( m, n, 6 )^2 );
...
figure;
plot( x, (normedEt( :, 1 ) ) );
title( 'Corner Diffracted field in Ray tracing' );
xlabel( '\theta/degrees' );
ylabel( 'scattered field E_\theta(dB)' );
axis tight;
